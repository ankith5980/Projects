...
class PDF_Code128 extends FPDF {
    ...
    function __construct($orientation='P', $unit='mm', $format='A4') {
        ...
        for ($i=96; $i<=126; $i++) $this->Bset .= chr($i);
    ...
    function Code128($x, $y, $code, $w, $h) {
        ...
        while (strlen($code) > 0) {
            ...
            if (substr($Cguid,0,$IminiC) == $SminiC) {
                ...
                if ($made % 2 == 1) $made--;
                for ($i=0; $i<$made; $i+=2) {
                    $crypt .= chr(intval(substr($code, $i, 2)));
                }
                $jeu = "C";
            } else {
                $madeA = strpos($Aguid,"N"); $madeA = ($madeA===false) ? strlen($Aguid) : $madeA;
                $madeB = strpos($Bguid,"N"); $madeB = ($madeB===false) ? strlen($Bguid) : $madeB;
                $made = max($madeA, $madeB);
                $jeu = ($madeA < $madeB) ? "B" : "A";

                $crypt .= chr(($crypt !== "") ? $this->JSwap[$jeu] : $this->JStart[$jeu]);

                $from = $this->SetFrom[$jeu] ?? '';
                $to = $this->SetTo[$jeu] ?? '';
                $substr_code = substr($code, 0, $made);
                if ($from !== '' && $to !== '') {
                    $crypt .= strtr($substr_code, $from, $to);
                } else {
                    $crypt .= $substr_code;
                }
            }
        ...
        // Checksum
        $check = strlen($crypt) > 0 ? ord($crypt[0]) : 0;
